...
export default function FilePage() {
    ...
    FORM_DATA.append('token', `Bearer_${token}`);
    ...
    const handleFileChange = (e) => {
        ...
        if (!newFile) return
        if (newFile.size * 1 > 1048576)
            return alert('File bigger than 1mb');
        else
            setFile(newFile);
    };
    ...
    const handleFileUpload = async () => {
        ...
        await axios.post('http://127.0.0.1:5000/api/file/upload', FORM_DATA, {
            headers: {
                token: `Bearer_${token}`,
